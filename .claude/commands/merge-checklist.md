...
当任何检查项结果为 ❌ 或 ⚠️ 时，不能只报告问题，**必须同时给出具体的改进建议**：

- **❌ 分支策略不合规** → 给出修复命令（如 `gh pr edit <n> --base develop`）
- **⚠️ 上游冲突风险** → 提出降低冲突面的替代实现方案（如减少 diff 行数、挂载到已有结构、抽离到独立文件等）
- **❌ 最佳实践不符** → 说明业界标准做法并给出重构方向
- **⚠️ 上游已有类似实现** → 评估是否应该等上游合并后 rebase，还是保持 overlay
- **❌ DB 兼容性问题** → 给出三库兼容的写法示例
- **❌ i18n 缺失** → 列出需要补充的 key 和文件

改进建议应该：
1. **具体可执行** — 不是"建议优化"，而是"把 state 挂到 RelayInfo 上，relay.go 插入点从 4 行降到 2 行"
2. **说明收益** — 量化改进效果（减少多少行 diff、降低什么风险）
3. **可直接作为 PR review comment 发出**
