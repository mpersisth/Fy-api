...
| 03b | `03b-add-redirect-domain.sh` | 给次要域名配 HTTPS + 301 跳到主域(如 www → api,地址栏会变) | 需要「自动跳转」时 |
| 03c | `03c-add-alias-domain.sh` | 给别名域名配 HTTPS + 直接反代(如 www 和 api 并列,地址栏不变) | 需要「多域名并存」时 |
