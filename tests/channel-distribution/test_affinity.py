...
from lib.log_query import query_distribution_by_token
...
def run_affinity_sticky_test(
    ...
    """Test: Rule matching and sticky routing."""
    channels = [
        ChannelConfig(name="aff-A", priority=1000, weight=50),
        ChannelConfig(name="aff-B", priority=1000, weight=50),
    ]
    ...
    result.scenarios.append(AffinityScenarioResult(
        name=f"Same key ({same_key}) x {num_requests} -> single channel",
        passed=sticky,
    ...
    for r in diff_results:
        ch = _get_channel_for_request(client, r.request_id)
        if ch > 0:
            channels_diff.add(ch)
    distributed = len(channels_diff) >= 2
    result.scenarios.append(AffinityScenarioResult(
        name=f"Different keys x {num_requests} -> distributed",
        passed=distributed,
        detail=f"Channels used: {len(channels_diff)} ({[ctx.channel_names.get(c, c) for c in channels_diff]})",
    ))
...
def run_affinity_ttl_test(
    ...
    """Test: TTL expiry and renewal behavior."""
    channels = [
        ChannelConfig(name="ttl-A", priority=1000, weight=50),
        ChannelConfig(name="ttl-B", priority=1000, weight=50),
    ]
    ctx = harness.setup_channels(channels)
    print(f"  Created channels: {ctx.channel_names}")
    ...
    print("  Sending requests after TTL expiry with new keys...")
    harness.clear_affinity_cache()
    time.sleep(0.5)
    for _ in range(10):
        new_key = f"ttl-post-{uuid.uuid4().hex[:8]}"
        _send_with_header_key(client, ctx.token_key, harness.model, new_key, 1)
        time.sleep(0.1)
    time.sleep(log_wait)
    result.scenarios.append(AffinityScenarioResult(
...
def run_affinity_failure_test(
    ...
    """Test: SkipRetryOnFailure and SwitchOnSuccess behavior."""
    channels = [
        ChannelConfig(name="fail-A", priority=1000, weight=50),
        ChannelConfig(name="fail-B", priority=1000, weight=50),
    ]
    ctx = harness.setup_channels(channels)
    print(f"  Created channels: {ctx.channel_names}")
...
def run_affinity_hit_rate_test(
    ...
    """Test: Hit rate analysis under different access patterns."""
    channels = [
        ChannelConfig(name="hr-A", priority=1000, weight=50),
        ChannelConfig(name="hr-B", priority=1000, weight=50),
    ]
    ctx = harness.setup_channels(channels)
    print(f"  Created channels: {ctx.channel_names}")
    ...
    print("  F1: Long session (same key x 30)...")
    ...
    result.scenarios.append(AffinityScenarioResult(
        name="Long session (same key x 30)",
    ...
    print("  F2: Cold start (20 unique keys x 1)...")
    harness.clear_affinity_cache()
    time.sleep(0.5)
    for i in range(20):
        key_f2 = f"cold-{uuid.uuid4().hex[:8]}"
        _send_with_header_key(client, ctx.token_key, harness.model, key_f2, 1)
        time.sleep(0.05)
    result.scenarios.append(AffinityScenarioResult(
        name="Cold start (20 unique keys x 1 each)",
        passed=True,
        hit_rate=0.0,
    ...
    print("  F3: Returning users (10 keys x 5 each)...")
    harness.clear_affinity_cache()
    time.sleep(0.5)
    results_f3 = []
    ...
    result.scenarios.append(AffinityScenarioResult(
        name="Returning users (10 keys x 5 each)",
